...
**With no labels written every occasion must read `Unchanged`.** That was the
zero-label equivalence check as a live page. It is spent — the confirmed buggy
labels ended it, which is what this page now exists to show.
"""
...
def render_impact(version: str) -> None:
    ...
    new, legacy = load_new(version), load_legacy(version)
