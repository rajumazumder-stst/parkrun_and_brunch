...
| `tests/` | pytest suite, run from the repo root. **pytest is dev-only, deliberately not in `requirements.txt`** (same convention as `openpyxl` and `cairosvg`). Currently covers `buggy_estimator` only — 34 cases, no database |
